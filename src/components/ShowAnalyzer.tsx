...
export const ShowAnalyzer = ({ show }: { show: string }) => {
  ...
  const analyzeResponses = async () => {
    ...
    try {
      ...
      if (data.result.confidence < 0.7 && data.result.followUpQuestions) {
        setState(prev => ({
          ...prev,
          questions: [
            ...prev.questions,
            ...data.result.followUpQuestions.map((q: string) => ({
              question: q,
              answer: null,
            })),
          ],
        ...
        {isLoadingQuestions ? (
          <div className="flex items-center justify-center p-3 xs:p-4 sm:p-8">
            <div className="flex items-center gap-2 xs:gap-3">
              <Loader2 className="w-4 xs:w-5 sm:w-6 h-4 xs:h-5 sm:h-6 animate-spin text-orange-500" />
              <p className="text-sm xs:text-base sm:text-lg text-orange-200/70">Generating questions...</p>
            </div>
          </div>
        ) : state.error ? (
          ...
          <div className="px-0 sm:px-4">
            <motion.div
              ...
              exit={{ opacity: 0, y: -20 }}
            >
              <Card className="p-3 xs:p-4 sm:p-8 bg-gray-800/60 border-orange-900/30 shadow-xl backdrop-blur-sm radial-gradient-subtle">
              ...
              <div className="px-0 sm:px-4">
                ...
                >
                  <Card className="p-3 xs:p-4 sm:p-8 bg-gray-800/60 border-orange-900/30 shadow-xl backdrop-blur-sm">
                    <h3 className="text-sm xs:text-base sm:text-xl font-medium mb-3 xs:mb-4 sm:mb-6 text-orange-50">{currentQuestion.question}</h3>
                      ...
                      {answerButtons.map(({ value, icon, label, color }) => (
                        <Button
                          ...
                          className={`h-9 xs:h-10 sm:h-12 py-2 xs:py-2.5 sm:py-3 px-3 xs:px-4 sm:px-6 flex items-center justify-center gap-2 xs:gap-3 text-white transition-all shadow-lg shadow-orange-900/20
                            ${currentQuestion.answer === value ? color : 'bg-gray-700/80 hover:bg-gray-700 border border-orange-900/30'}`}
                        >
                          {icon}
                      ...
                      >
                        <div className="flex items-center gap-2 mb-2 xs:mb-3 sm:mb-4">
                          <Button
                            ...
                            className="text-xs xs:text-sm text-orange-200/70 hover:text-orange-50 w-full sm:w-auto py-1.5 xs:py-2 h-auto"
                          >
